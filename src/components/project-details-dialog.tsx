        ...
        <ScrollArea className="h-[calc(90vh-120px)] mt-4">
          <div className="prose prose-blue max-w-none prose-p:my-4 prose-li:my-1 prose-headings:mt-6 prose-headings:mb-4 prose-table:my-4 pr-4">
            <Markdown
              ...
              rehypePlugins={[rehypeRaw, rehypeSlug, rehypeSanitize, [rehypeExternalLinks, { target: "_blank", rel: ["nofollow", "noopener", "noreferrer"] }]]}
              components={{
                table: ({ ...props }) => (
                ...
                th: ({ ...props }) => (
                  <th className="border border-slate-300 bg-slate-100 p-2 font-semibold" {...props} />
                ),
                td: ({ ...props }) => (
